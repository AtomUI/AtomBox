...
namespace AtomBox.Providers.ObjectStorage.QingStor;

public sealed class QingStorProviderCreator : IStorageProviderCreator
{
    private readonly ProviderCredentialResolver _credentialResolver;

    public QingStorProviderCreator(ProviderDescriptor descriptor, ProviderCredentialResolver credentialResolver)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return OperationResult<IStorageProvider>.Failure(StorageError.Validation("QingStor endpoint is required."));
        }

        var region = account.GetProviderConfigValue("region");
        if (string.IsNullOrWhiteSpace(region))
        {
            return OperationResult<IStorageProvider>.Failure(StorageError.Validation("QingStor zone is required."));
        }
    ...
    private static OperationResult ValidateSecretMaterial(CredentialSecretMaterial material)
    {
        ...
        {
            return OperationResult.Failure(StorageError.Validation("QingStor credential requires accessKeyId."));
        }

        if (!material.TryGetValue("accessKeySecret", out _))
        {
            return OperationResult.Failure(StorageError.Validation("QingStor credential requires accessKeySecret."));
        }
