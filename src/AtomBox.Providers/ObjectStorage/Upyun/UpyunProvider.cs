...
public sealed class UpyunProvider : IStorageProvider
{
    private readonly IUpyunClient _client;
    private readonly CredentialMaterialLease _credentialLease;

    internal UpyunProvider(
        IUpyunClient client,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var items = ListObjects(path, pageRequest.SearchPrefix);
            return items.IsFailure
                ? Task.FromResult(OperationResult<RemoteItemPage>.Failure(items.Error!))
                : Task.FromResult(ProviderPaging.PageInMemory(path, items.GetValueOrThrow(), pageRequest));
        }
        catch (Exception exception)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            var pathResult = UpyunPath.FromRemotePath(path);
            if (pathResult.IsFailure)
            {
                return Task.FromResult(OperationResult.Failure(pathResult.Error!));
            }

            var upyunPath = pathResult.GetValueOrThrow();
            if (string.IsNullOrWhiteSpace(upyunPath.KeyPrefix))
            {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            else
            {
                ...
                _client.PutObject(upyunPath.BucketName, upyunPath.KeyPrefix, progressStream);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var upyunPath = pathResult.GetValueOrThrow();
            using var upyunObject = _client.GetObject(upyunPath.BucketName, upyunPath.KeyPrefix);
            await CopyToAsync(upyunObject.Content, destination, upyunObject.ContentLength, progress, cancellationToken)
    ...
    public Task<OperationResult> CreateFolderAsync(RemotePath path, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _client.PutObject(upyunPath.BucketName, upyunPath.ToFolderPrefix(), empty);
            return Task.FromResult(OperationResult.Success());
        }
        catch (Exception exception)
        {
    ...
    private OperationResult<IReadOnlyList<RemoteItem>> ListObjects(RemotePath path, string? searchPrefix = null)
    {
        var pathResult = UpyunPath.FromRemotePath(path);
        if (pathResult.IsFailure)
        {
        ...
        var upyunPath = pathResult.GetValueOrThrow();
        var prefix = upyunPath.ToFolderPrefix();
        var listPrefix = ObjectStorageSearchPrefix.Combine(prefix, searchPrefix);
        var listing = _client.ListObjects(upyunPath.BucketName, listPrefix);
        return OperationResult<IReadOnlyList<RemoteItem>>.Success(ToRemoteItems(upyunPath.BucketName, prefix, listing));
    }

    private IReadOnlyList<RemoteItem> ToRemoteItems(string bucketName, string prefix, UpyunObjectListing listing)
    {
    ...
    private static OperationResult<UpyunPath> GetObjectPath(RemotePath path, string emptyKeyMessage)
    {
        var pathResult = UpyunPath.FromRemotePath(path);
        if (pathResult.IsFailure)
        {
        ...
        var upyunPath = pathResult.GetValueOrThrow();
        return string.IsNullOrWhiteSpace(upyunPath.KeyPrefix)
            ? OperationResult<UpyunPath>.Failure(StorageError.Validation(emptyKeyMessage))
            : OperationResult<UpyunPath>.Success(upyunPath);
    }

    private void MoveFolder(UpyunPath source, UpyunPath destination)
    {
