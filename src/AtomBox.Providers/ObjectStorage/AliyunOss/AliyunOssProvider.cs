...
{
    private readonly IAliyunOssClient _client;
    private readonly CredentialMaterialLease _credentialLease;

    internal AliyunOssProvider(
        IAliyunOssClient client,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (!ProviderPageCursor.TryGetProviderToken(pageRequest.Cursor, "aliyun-oss", out var cursor))
            {
            ...
            var pathResult = AliyunOssPath.FromRemotePath(path);
            if (pathResult.IsFailure)
            {
                return Task.FromResult(OperationResult<RemoteItemPage>.Failure(pathResult.Error!));
            }

            var ossPath = pathResult.GetValueOrThrow();
            var prefix = ossPath.ToFolderPrefix();
            ...
            do
            {
                var listing = _client.ListObjects(ossPath.BucketName, listPrefix, nextToken, pageRequest.PageSize);
                items.AddRange(ToRemoteItems(ossPath.BucketName, prefix, listing));
            ...
            RemotePageCursor? nextCursor = string.IsNullOrWhiteSpace(nextToken)
                ? null
                : ProviderPageCursor.FromProviderToken("aliyun-oss", nextToken);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var ossPath = pathResult.GetValueOrThrow();
            if (string.IsNullOrWhiteSpace(ossPath.KeyPrefix))
            {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            else
            {
                ...
                _client.PutObject(ossPath.BucketName, ossPath.KeyPrefix, progressStream);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var ossPath = pathResult.GetValueOrThrow();
            using var ossObject = _client.GetObject(ossPath.BucketName, ossPath.KeyPrefix);
            await CopyToAsync(
                ossObject.Content,
                destination,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _client.PutObject(ossPath.BucketName, ossPath.ToFolderPrefix(), empty);
            return Task.FromResult(OperationResult.Success());
        }
        catch (Exception exception)
        {
    ...
    private OperationResult<IReadOnlyList<RemoteItem>> ListObjects(RemotePath path)
    {
        ...
        var ossPath = pathResult.GetValueOrThrow();
        var prefix = ossPath.ToFolderPrefix();
        var listing = _client.ListObjects(ossPath.BucketName, prefix);
        return OperationResult<IReadOnlyList<RemoteItem>>.Success(ToRemoteItems(ossPath.BucketName, prefix, listing));
    }

    private IReadOnlyList<RemoteItem> ToRemoteItems(string bucketName, string prefix, AliyunOssObjectListing listing)
    {
    ...
    private static OperationResult<AliyunOssPath> GetObjectPath(RemotePath path, string emptyKeyMessage)
    {
        var pathResult = AliyunOssPath.FromRemotePath(path);
        if (pathResult.IsFailure)
        {
        ...
        if (string.IsNullOrWhiteSpace(ossPath.KeyPrefix))
        {
            return OperationResult<AliyunOssPath>.Failure(StorageError.Validation(emptyKeyMessage));
        }

        return OperationResult<AliyunOssPath>.Success(ossPath);
    }

    private bool ObjectExists(AliyunOssPath path)
    {
        var listing = _client.ListObjects(path.BucketName, path.KeyPrefix);
        return listing.Objects.Any(item => string.Equals(item.Key, path.KeyPrefix, StringComparison.Ordinal));
    }

    private void MoveFolder(AliyunOssPath source, AliyunOssPath destination)
    {
