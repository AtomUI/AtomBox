...
> 测试框架：xUnit
> 最新更新：2026-06-22，v0.1 发布前文档同步确认；保留 2026-06-19 `CredentialStore` 并发租约修复说明

## 1. 总体结果

- **总计：147 项测试，0 失败，0 跳过，0 警告**
- 测试用时：507ms
- 构建验证：`dotnet build AtomBox.slnx` — 0 warning / 0 error
- 最新验证：`dotnet test tests\AtomBox.Infrastructure.Tests\AtomBox.Infrastructure.Tests.csproj --no-restore` — 147 passed / 0 failed
...
| `CredentialStoreContractTests` | 18 | 保存/获取凭据、租约生命周期、待删除、多租约、索引秘密、pending-delete 并发边界 |
...
- **问题：** `AcquireLeaseAsync` 在 `ExistsAsync` 和 `CreateLease` 之间存在 TOCTOU 窗口；`AcquireMaterialAsync` 在读取凭据材料和创建租约之间存在 stale-read 窗口，`MarkPendingDeleteAsync` 可在窗口期插入。
- **修复：** 新增凭据级异步门闩，将凭据索引读取、pending-delete 判断和租约创建收进同一临界区；`MarkPendingDeleteAsync` 也在同一门闩下更新索引，确保 pending-delete 不能插入检查与租约创建之间。
- **验证：** 新增 `AcquireLease_PendingDelete_ReturnsNotFound`、`AcquireLease_DoesNotInterleavePendingDeleteBeforeLeaseCreation`、`AcquireMaterial_DoesNotInterleavePendingDeleteBeforeLeaseCreation`，并通过 Infrastructure 全量测试。
...
6. `CredentialStore` 使用独立的凭据级异步门闩保护索引状态与租约授予原子性；`_leaseGate` 仍只负责内存租约计数。
